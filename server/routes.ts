...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      ...
      const updatedItem = await db
        ...
        .set({
          ...
          isGift: parseResult.data.isGift,
  ...
  app.delete("/api/items/:id", async (req, res) => {
    try {
      ...
      await db.delete(schema.itemRequests).where(eq(schema.itemRequests.itemId, itemId));
      await db.delete(schema.itemBids).where(eq(schema.itemBids.itemId, itemId));
      await db.delete(schema.favoriteTable).where(eq(schema.favoriteTable.itemId, itemId));

      // Then delete the item
      await db.delete(schema.items).where(eq(schema.items.id, itemId));

      logger.debug('Deleted item and related records:', { itemId });
