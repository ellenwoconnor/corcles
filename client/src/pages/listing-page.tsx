...
import { Loader2, MessageSquare, Edit, Trash2, Check, X, Clock } from "lucide-react";
...
function PickupConfirmation({ item, request }: { item: Item; request: ItemRequest }) {
  ...
  if (!item.pickupStart || !item.pickupEnd) {
    return null;
  }
    ...
    <div className="space-y-4 border-t border-border pt-4">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          {formatDistanceToNow(new Date(item.pickupStart), { addSuffix: true })} -{" "}
          {formatDistanceToNow(new Date(item.pickupEnd), { addSuffix: true })}
        </p>
      </div>
